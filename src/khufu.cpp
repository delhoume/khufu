...
void cbError(struct mg_str& uri) {
  MG_ERROR(("%.*s -=> %s", uri.len, uri.ptr, error));
}
...
static void cb(struct mg_connection *c, int ev, void *ev_data){
  if (ev == MG_EV_HTTP_MSG) {
    ...
    uLong signature = adler32(0, (const Bytef *)caps[0].ptr, caps[0].len);
    TIFF *tifin;
    ...
    if (level >= cache.num_directories) {
      mg_snprintf(error, sizeof(error), "directory number %d exceed limit %d", level, cache.num_directories - 1);
      cbError(uri);
      return;
    }
